...
function isCreditCardNumber(ccn) {
  ...
  const originalCheckDigit = digits[len - 1];
...
function getCommonDirectoryPath(pathes) {
  ...
  const first = splitPathes[0];
  const commonSegments = [];
  for (let i = 0; i < first.length; i += 1) {
    const segment = first[i];
    const isCommon = splitPathes.every((segments) => segments[i] === segment);
    if (isCommon) {
  ...
  if (commonSegments[0] === '' && commonSegments.length === 1) return '/';
